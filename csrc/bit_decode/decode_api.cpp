...
mha_fwd_kvcache(at::Tensor &q,                       // batch_size x seqlen_q x num_heads x head_size
    ...
    at::Tensor k, v, k_pack_new, k_params_new, v_pack_new, v_params_new;
    if (k_.has_value()) {
        ...
        if (quant_mode == "k-channel") {
            k_pack_new   = torch::empty({batch_size, residual_block_size / pack_nums, num_heads_k, k_pack.size(-1)}, torch::dtype(torch::kUInt16)).to(torch::kCUDA);
            k_params_new = torch::empty({batch_size, k_params.size(1), num_heads_k, residual_block_size / group_size}, torch::dtype(torch::kFloat32)).to(torch::kCUDA);
        } else {
            k_pack_new   = torch::empty({batch_size, residual_block_size,              num_heads_k, k_pack.size(-1)}, torch::dtype(torch::kUInt16)).to(torch::kCUDA);
            k_params_new = torch::empty({batch_size, k_params.size(1), num_heads_k, residual_block_size    }, torch::dtype(torch::kFloat32)).to(torch::kCUDA);
        }

        v_pack_new   = torch::empty({batch_size, residual_block_size,              num_heads_k, v_pack.size(-1)}, torch::dtype(torch::kUInt16)).to(torch::kCUDA);
        v_params_new = torch::empty({batch_size, v_params.size(1), num_heads_k, residual_block_size    }, torch::dtype(torch::kFloat32)).to(torch::kCUDA);
        ...
        params.k_params_new_row_stride   = k_params_new.stride(-1);
        ...
        params.k_params_new_dim_stride   = k_params_new.stride(-3);
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.doc() = "BitDecoding";
    // Expose specific template instantiations for 2-bit and 4-bit versions
