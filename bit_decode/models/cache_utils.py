...
class DynamicCache(Cache):
    ...
    def __getitem__(self, layer_idx: int) -> List[Tuple[torch.Tensor]]:
        ...
        if layer_idx < len(self):
            if len(self.key_cache) > 0:
                return (self.key_cache[layer_idx], self.value_cache[layer_idx], self.key_cache_pack[layer_idx], self.key_cache_params[layer_idx], self.value_cache_pack[layer_idx], self.value_cache_params[layer_idx])
            else:
    ...
    def __len__(self):
        ...
        """
        return len(self.key_cache_pack) if len(self.key_cache_pack) > 0 else len(self.key_cache)
    ...
    def update(
        ...
        if key_states is not None:
            if len(self.key_cache) <= layer_idx:
                ...
                self.key_cache[layer_idx] = torch.cat([self.key_cache[layer_idx], key_states], dim=dim)
                self.value_cache[layer_idx] = torch.cat([self.value_cache[layer_idx], value_states], dim=dim)
    ...
    def get_seq_length(self, layer_idx: Optional[int] = 0) -> int:
        ...
        layer_seq_length = self.key_cache[layer_idx].shape[-3] if not is_empty_layer else 0
    ...
    def to_legacy_cache(self) -> Tuple[Tuple[torch.Tensor], Tuple[torch.Tensor]]:
        ...
        for layer_idx in range(len(self)):
            if len(self.key_cache) == 0:
                legacy_cache += ((self.key_cache_pack[layer_idx], self.key_cache_params[layer_idx], self.value_cache_pack[layer_idx], self.value_cache_params[layer_idx]),)
            elif len(self.key_cache_pack) > 0:
                legacy_cache += ((self.key_cache[layer_idx], self.value_cache[layer_idx], self.key_cache_pack[layer_idx], self.key_cache_params[layer_idx], self.value_cache_pack[layer_idx], self.value_cache_params[layer_idx]),)
            else:
    ...
    def from_legacy_cache(
        ...
        if past_key_values is not None:
            for layer_idx in range(len(past_key_values)):
                ...
                if len(past_key_values[layer_idx]) == 6:
                    k_state_residual, v_state_residual, k_pack, k_params, v_pack, v_params = past_key_values[layer_idx]
                    cache.update_residual(k_state_residual, v_state_residual, layer_idx)
                    cache.update_pack(k_pack, k_params, v_pack, v_params, layer_idx)
                elif len(past_key_values[layer_idx]) == 4:
                    k_pack, k_params, v_pack, v_params = past_key_values[layer_idx]
                    cache.update_pack(k_pack, k_params, v_pack, v_params, layer_idx)
                else:
                    k_state, v_state = past_key_values[layer_idx]
                    cache.update(k_state, v_state, layer_idx)
